...
import { CalendarRange, Goal, Mail, MessageSquare, Users } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { saveQuarterlyGoals } from '@/lib/analytics/quarterly-actions';

const QUARTERS = ['Q1', 'Q2', 'Q3', 'Q4'] as const;
export type QuarterKey = (typeof QUARTERS)[number];

export type QuarterlyEmailRow = { sent_at: Date; reply_count: number | null };
export type QuarterlyMeetingRow = { created_at: Date; meeting_status: string | null };
export type QuarterlyCampaignRow = {
  name: string;
  status: string;
  created_at: Date;
  start_date: Date | null;
};
export type QuarterlyAccountRow = {
  name: string;
  pipeline_stage: string | null;
  priority_score: number | null;
  updated_at: Date;
};
export type QuarterlyGoalConfig = { key: string; value: string };

type QuarterlyRichProps = {
  emails: QuarterlyEmailRow[];
  meetings: QuarterlyMeetingRow[];
  campaigns: QuarterlyCampaignRow[];
  accounts: QuarterlyAccountRow[];
  goalConfigs: QuarterlyGoalConfig[];
};
...
export function QuarterlyRich({
  ...
  const quarterRows = QUARTERS.map((quarter, index) => {
    ...
    const quarterEmails = emails.filter((e) => e.sent_at >= start && e.sent_at < end);
    const quarterMeetings = meetings.filter((m) => m.created_at >= start && m.created_at < end);
    const quarterCampaigns = campaigns.filter((c) => {
      const stamp = c.start_date ?? c.created_at;
      return stamp >= start && stamp < end;
    });
    const quarterAccounts = accounts.filter((a) => a.updated_at >= start && a.updated_at < end);
    const dealsClosed = quarterAccounts.filter((a) => a.pipeline_stage === 'closed').length;
    const estimatedPipeline = quarterAccounts
      .filter((a) => ['meeting', 'proposal', 'closed'].includes((a.pipeline_stage ?? '').toLowerCase()))
      .reduce((sum, a) => sum + Math.max(10000, (a.priority_score ?? 0) * 1000), 0);
    const goals = goalMap.get(quarter) ?? { meetingTarget: 0, pipelineTarget: 0 };

    return {
      quarter,
      sends: quarterEmails.length,
      replies: quarterEmails.reduce((sum, e) => sum + (e.reply_count ?? 0), 0),
      meetings: quarterMeetings.filter((m) => (m.meeting_status ?? '').toLowerCase() !== 'no meeting').length,
      dealsClosed,
      estimatedPipeline,
  ...
  const activeQuarter = quarterRows.find((q) => q.isCurrent) ?? quarterRows[0];
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="rounded-lg border p-3">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {quarter.dealsClosed} closed this quarter
                </p>
              </div>
              <div className="space-y-2 text-xs text-muted-foreground">
                <p>Meeting target: {quarter.meetings}/{quarter.goals.meetingTarget || 0}</p>
                <div className="h-2 rounded-full bg-muted">
                  <div
                    className="h-2 rounded-full bg-cyan-600"
                    style={{ width: `${percentOf(quarter.meetings, quarter.goals.meetingTarget)}%` }}
                  />
                </div>
                <p>Pipeline target: {formatCurrency(quarter.estimatedPipeline)} / {formatCurrency(quarter.goals.pipelineTarget || 0)}</p>
                <div className="h-2 rounded-full bg-muted">
                  <div
                    className="h-2 rounded-full bg-emerald-600"
                    style={{ width: `${percentOf(quarter.estimatedPipeline, quarter.goals.pipelineTarget)}%` }}
                  />
                </div>
              </div>
            </CardContent>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
            ...
            <form action={saveQuarterlyGoals} className="space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    type="number"
                    min={0}
                    defaultValue={activeQuarter.goals.meetingTarget || 10}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    min={0}
                    step={5000}
                    defaultValue={activeQuarter.goals.pipelineTarget || 250000}
                  />
                </div>
              </div>
              <div className="flex justify-end">
                <Button type="submit" className="gap-1.5">
                  ...
                  Save {activeQuarter.quarter} Goals
                </Button>
              </div>
            </form>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm text-muted-foreground">
            <p>Keep the board simple: more replies, more meetings, more pipeline value moving toward proposal.</p>
            <div className="rounded-lg border p-3">
              <p className="text-[10px] uppercase text-muted-foreground">Current quarter</p>
              <p className="mt-1 text-xl font-bold text-foreground">
                {activeQuarter.quarter} {year}
              </p>
              <p className="mt-1 text-xs">
                Sends {activeQuarter.sends} · Replies {activeQuarter.replies} · Meetings {activeQuarter.meetings}
              </p>
            </div>
            <div className="rounded-lg border p-3">
              <p className="text-[10px] uppercase text-muted-foreground">Pipeline target</p>
              <p className="mt-1 text-xl font-bold text-foreground">
                {formatCurrency(activeQuarter.goals.pipelineTarget || 0)}
              </p>
