...
import {
  runReenrichContactsNowAction,
  runSyncHubspotDryRunNowAction,
} from '@/lib/admin/cron-runbook-actions';
import {
  DRIP_SEQUENCE_ENABLED,
  HUBSPOT_LOGGING_ENABLED,
  HUBSPOT_SYNC_ENABLED,
  INBOX_POLLING_ENABLED,
} from '@/lib/feature-flags';
import { KNOWN_CRONS, type CronStateValue } from '@/lib/cron-monitor';

export type CronSystemConfigRow = { key: string; value: string };
export type CronGenerationJobRow = {
  status: string;
...
type CronHealthRichProps = {
  configs: CronSystemConfigRow[];
  generationJobs: CronGenerationJobRow[];
};
...
export function CronHealthRich({ configs, generationJobs }: CronHealthRichProps) {
  ...
  const inFlightGenJobs = generationJobs.filter(
  ...
  const avgCompletionMs = recentCompletedDurations.length > 0
    ? Math.round(recentCompletedDurations.reduce((sum, d) => sum + d, 0) / recentCompletedDurations.length)
    : 0;
  ...
  return (
    <div className="space-y-4">
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-bold text-amber-600">
              {rows.filter((row) => row.state.status === 'error' || row.state.status === 'skipped').length}
            </p>
      ...
      {hubspotIngestionAudit ? (
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm">HubSpot Ingestion Audit</CardTitle></CardHeader>
          <CardContent className="grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="rounded-lg border p-3">
              ...
              <p className="mt-1 font-medium">
                {hubspotIngestionAudit.cursorBefore ?? 'start'} → {hubspotIngestionAudit.cursorAfter ?? 'end'}
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-sm">Runbook Actions</CardTitle></CardHeader>
            ...
            <CardContent className="space-y-3 text-sm">
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="rounded-lg border p-3">
                  ...
                  <p className="mt-1 font-medium">
                    {row.state.lastDurationMs ? `${row.state.lastDurationMs} ms` : '—'}
                  </p>
                </div>
              </div>
              <div className="rounded-lg border p-3">
              ...
              <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                ...
                <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-1">
                  <Clock3 className="h-3 w-3" /> last success:{' '}
                  {row.state.lastSuccessAt ? new Date(row.state.lastSuccessAt).toLocaleDateString() : '—'}
                </span>
              </div>
              {row.state.lastStats ? (
                <pre className="overflow-auto rounded-lg bg-muted/40 p-3 text-xs">
                  {JSON.stringify(row.state.lastStats, null, 2)}
                </pre>
              ) : null}
