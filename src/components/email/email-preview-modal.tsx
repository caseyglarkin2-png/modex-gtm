      ...
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
        ...
        <div className="flex-1 overflow-auto space-y-4">
          ...
          {showRateLimitWarning && (
            <div className="px-3 py-2 bg-blue-500/10 border border-blue-500/20 rounded-md flex items-start gap-2">
              <Info className="h-4 w-4 text-blue-600 mt-0.5 shrink-0" />
              <div className="text-xs text-blue-700 dark:text-blue-400">
                <p className="font-semibold">{rateLimitRemaining} emails remaining this minute</p>
                <p>Rate limit: 10 emails per minute to prevent abuse.</p>
              </div>
            </div>
          )}
        ...
        <div className="flex items-center justify-between pt-4 border-t">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            {rateLimitRemaining !== undefined && (
              <Badge variant={rateLimitRemaining <= 3 ? 'default' : 'secondary'} className="font-mono">
