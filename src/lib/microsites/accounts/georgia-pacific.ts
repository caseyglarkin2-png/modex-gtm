 ...
 * Pitch shape: dual-customer-flow yard-ops wedge. Same mill ships rolls of
 * Quilted Northern toward retail DCs and pallets of plywood/OSB toward
 * contractors via distributor networks. Two customer types = two appointment
 * cadences, two dock-arbitration rules, two carrier mixes — surfaced at the
 * yard layer of facilities that run both. Koch Industries' Market-Based
 * Management surfaces operational variance to a degree most parent companies
 * don't, which means the variance is visible to leadership but the operating
 * layer above the yard that would make the dual-flow deterministic has not
 * been built yet. Framed observationally — G-P is privately held inside
 * Koch, so public material is thinner than for a public-co peer.
 */
...
export const georgiaPacific: AccountMicrositeData = {
  ...
  sections: [
    ...
    },
    {
      ...
      authorBio:
        'Casey Larkin builds YardFlow at FreightRoll. The brief above is a working analysis, not a sales asset — it is the same shape of memo we would circulate internally before sizing a network engagement. Georgia-Pacific is distinctive in this round for two related reasons. The first is the dual-customer-flow structure: the same operating company ships rolls of Quilted Northern toward retail DCs and pallets of OSB toward contractors, sometimes from the same site, and the dock-arbitration logic that gets two appointment cadences and two carrier mixes through the same gate is genuinely harder than at a single-flow plant. The second is the Koch Industries operating context. Market-Based Management is, by public record and by reputation, one of the most operationally measured frameworks in American industry — fifteen thousand production ML models tuning plant settings is the signature of a parent company that has chosen to push metrics far down the organization. That posture makes the unsolved yard layer at the dual-flow sites visible in a particular way: the variance exists, the operating philosophy is set up to see it, and the gate-to-dock layer above the sites that would make the dual-flow deterministic across the network has not, from the public record, been built yet. The Primo comparable covers the operating-model shape for either flow on its own; the arbitration-between-flows is its own design conversation.',
      authorEmail: 'casey@freightroll.com',
      signOff:
        'If parts of this read wrong against what you see internally — particularly which sites today actually run dual-flow at material scale, whether the gate-side digital posture already feeds an operating layer above the sites that we cannot see, or whether the Palatka 2026 ground-breaking is being designed against an explicit yard-layer spec — that\'s the most useful thing to push back on. The next step that makes sense is whatever the analysis prompts, not necessarily a meeting.',
    },
  ],
  ...
  personVariants: [
    {
      ...
      framingNarrative:
        'Ryan, Georgia-Pacific is one of the only operating companies of its scale that runs consumer tissue and contractor-bound building products through the same network — sometimes through the same site. The dock-arbitration logic at the dual-flow sites is genuinely harder than at a single-flow plant, and the Koch Market-Based Management posture is set up to see the variance that creates. The operating layer above the yard that would make the dual-flow deterministic is the unsolved seam in an otherwise aggressively-digitized supply chain.',
      openingHook:
        'The driver app at the gate, the SAS plant models behind the walls, the Point A center in Atlanta — the inside of the building has been digitized hard. The dock-door arbitration logic at a site that loads Quilted Northern toward a retail-DC appointment and OSB toward a distributor walk-in is the layer that hasn\'t been built out the same way. That asymmetry is what the analysis below works through.',
      stakeStatement:
        'Throughput is climbing where the capex is landing first — Palatka in February 2026, the sustainable-packaging build that closed in 2025, the Anchor Packaging integration. Throughput-out-the-door becomes trailer-into-the-yard. At a dual-flow site, an undecided dock door is two appointment windows missed, not one.',

      heroOverride: {
        headline: 'The yard is where two customer types meet at the same dock.',
        subheadline:
          'Quilted Northern toward a retail-DC appointment, OSB toward a contractor walk-in, same mill, same gate. The dock-arbitration layer above that is the unsolved seam inside an otherwise heavily-digitized operating company.',
      },
      sectionOrder: ['yns-thesis', 'observation', 'comparable', 'methodology', 'about'],

      toneShift:
        'Peer-to-peer supply-chain-operator framing. Lead with the dual-customer-flow wedge — it is the analytically distinctive thing about Georgia-Pacific against either a single-flow CPG or a single-flow building-products peer. Treat Koch Market-Based Management as operating context, not as flattery; the framework is well-understood and the right register is matter-of-fact. Acknowledge the digital posture (Point A, SAS, 15,000 ML models, the driver app, CV QA) as the credible base it is, and locate the unsolved seam above it.',
      kpiLanguage: [
        ...
        'detention spend',
        'inbound load-weighing cadence',
      ],
      proofEmphasis:
        'Primo is the public comparable to cite — same heavy-trailer freight economics, same multi-site network shape, already running a network-level operating model on top of site-level yard systems. Be explicit that Primo is single-customer-type and the dual-customer-type wrinkle is Georgia-Pacific-specific.',
    },
    {
      person: {
        personaId: 'P-037',
  ...
  signals: {
    ...
    supplyChainInitiatives: [
      'Dual-business operating company — consumer tissue/towel and building products under one supply chain — creates the dual-customer-flow yard-arbitration question at sites that ship both.',
      'Koch Industries Market-Based Management is the parent-company operating context; the framework surfaces operational variance to leadership in a way most parent companies do not.',
      'Heavy digital posture inside the building (SAS + AWS, 15,000+ ML models, computer-vision inbound QA, driver-facing mobile app for purchase-order entry and load weighing) — the gate-to-dock layer at the site, and the network operating standard above the sites, is the unsolved seam.',
    ],
    urgencyDriver:
