...
export const toyota: AccountMicrositeData = {
  ...
  sections: [
    ...
    },
    {
      ...
      comparableProfile:
        "Primo Brands runs what is, by most operating measures, the hardest CPG freight in North America. Bottled water is heavy (a fully loaded trailer maxes out gross-vehicle weight before it maxes cube), low-margin (so every minute of yard waste is a margin point you can't recover with price), shipped across multi-temp (premium SKUs sit alongside ambient), and complicated by return logistics for refillable formats. Primo is also years ahead of every other CPG category on yard automation and digitization, they had to be. They run a multi-site bottling and distribution network with high-volume plants, drop-trailer yards, and refrigerated lanes feeding regional DCs, and they have layered a network-level yard operating model on top of their existing site-level yard systems. The Toyota operating profile is different in industry, similar in structural shape: multi-site, multi-supplier, with mature in-plant operating discipline (TPS) already in place, where the binding constraint on production capacity is the variance budget per minute rather than the variance budget per dollar. The freight comparison runs the opposite direction from the usual read-across, Primo is harder per case, but a TPS assembly plant has a tighter tolerance per minute than any beverage plant in the country. Both networks have the same underlying problem: site-level execution is mature, network-level yard discipline above the sites is the unsolved layer. If a network operating model can run on water, the operationally hardest CPG freight, the read-across to a JIT assembly network is a different vertical with the same shape, not a harder lift. The shape Primo built above its plants is the same shape TPS would extend above the dock: standardized work, sequenced flow, and stop-authority at the layer that today still runs on radios.",
      metrics: [
