...
export const cjLogisticsAmerica: AccountMicrositeData = {
  ...
  sections: [
    ...
    },
    {
      ...
      sources: [
        ...
        },
        {
          ...
          confidence: 'public',
          detail: 'Adams\' public framing of AI as a safety/engagement/skill-development lever first, productivity second. ~20-year tenure inside the DSC / CJ Logistics organization; Purdue BS Industrial Management + UCLA Anderson MBA; pre-DSC at Lucent Technologies as Engineering Project Manager.',
        },
        {
    ...
    },
    {
      ...
      signOff:
        'If parts of this read wrong against what you see internally for CJ Logistics America — particularly which TES-pipeline sites are most receptive right now, whether the Elwood Phase 2 yard-ops design is in the engineering conversation today, or how the productization economics would map to the OneTrack commercial template — that\'s the most useful thing to push back on. The next step that makes sense is whatever the analysis prompts: a TES scoping session, a 90-day pilot conversation at one customer-funded site, or an integration architecture review with the engineering team — not necessarily a vendor demo.',
    },
  ],
  ...
  personVariants: [
    {
      ...
      framingNarrative:
        'Laura, the productization pattern your team has run for the past six years — identify a customer-side need, pilot a partner technology, validate KPIs (73% safety events, 11% UPH at OneTrack), scale across the network, productize for the customer base — is the pattern that turns the yard-execution layer into the next entry in the TES product catalog. The OneTrack precedent is the template; the WMS-agnostic architecture principle you have stated on the record is the discriminating differentiator the legacy YMS category cannot meet without re-engineering against its WMS-anchored architecture.',
      openingHook:
        'You scaled OneTrack from one site to 40+ in six years with measurable KPIs (73% safety events, 11% UPH lift, Feb 2025 expansion announcement). The yard-execution layer is the natural next entry in the TES catalog: multi-tenant from the data model up, WMS-agnostic by design (Blue Yonder + Korber + SAP EWM in parallel without re-engineering), and ready to embed at the Elwood Phase 2 greenfield + the NEXUS Project hubs at the design phase, not the retrofit phase.',
      stakeStatement:
        'Elwood Phase 2 (1.1M sq ft, H1 2026) and the additional NEXUS hubs are TES-engineered from day one. The new cold-chain sites (Gainesville, New Century) are multi-customer by definition. Each is a greenfield yard-ops design window — and specifying the network-tier orchestration layer at design is materially cheaper than retrofitting it after the building goes live. The OneTrack cadence is the proof that TES can productize at this rhythm; the yard-execution layer is the next adjacency.',
