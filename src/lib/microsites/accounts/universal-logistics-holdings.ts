  ...
  personVariants: [
    {
      ...
      framingNarrative:
        'Jeff, the LINC value-added services bid template — the document that decides what tools are in the standard playbook for the next OEM plant contract — is the surface a multi-tenant yard-execution layer fits into the same way the existing WMS estate does. LINC wins customer-by-customer on plant-side operating quality; GM Supplier of the Year three years running is the proof. The next visible differentiator OEMs are starting to ask about — real-time gate-to-dock orchestration, driver-facing workflows, shipper-visible feeds — is the layer above the records-layer YMS that ULH already names publicly.',
      openingHook:
        'LINC\'s day job is operating sequencing centers, sub-assembly plants, parts-marshalling yards, and returnable-container loops for OEMs — and the yard at every one of those operations is the operating system. The records-layer YMS ULH names publicly does what it was bought to do; the layer above — multi-tenant orchestration designed for the LINC multi-OEM operating model — is the next product-catalog entry for the value-added services bid template, billable through to the OEM as part of the operating contract.',
      stakeStatement:
        '~$150M capex planned for ULH in 2026 against capital-discipline framing; new CFO Michael Rogers in seat from June 1 through a period of internal-control remediation and segment repositioning. The procurement path to a corporate-IT YMS replacement is closed for the right reasons. The partnership path that flows through LINC customer billing — embedded in the value-added services pricing, sold as a LINC-branded capability inside the GM Supplier of the Year operating reputation — is the path open in 2026.',

      heroOverride: {
        headline: 'The yard-execution layer for LINC\'s value-added services bid template.',
        subheadline:
          'Multi-tenant by design (LINC operates plants for Stellantis, GM, and others simultaneously with different operating models per OEM). Embeddable into the value-added services bid template. Partnership economics through the LINC customer billing, not the ULH corporate tech budget. After Parsec — the natural next plant-side capability LINC can bid into the next OEM RFP.',
      },
