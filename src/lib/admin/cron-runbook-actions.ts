...
async function writeRunbookAudit(
  action: 'reenrich-contacts' | 'sync-hubspot',
  ...
  await prisma.systemConfig.upsert({
    ...
    update: { value: JSON.stringify({ action, mode, at, signature }) },
    create: { key: `runbook:${action}:last`, value: JSON.stringify({ action, mode, at, signature }) },
  });
}
