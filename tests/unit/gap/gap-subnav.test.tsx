...
describe('<GapSubnav>', () => {
  it('renders Queue, Hypotheses, Learning linking to the three canonical GAP surfaces', () => {
    render(<GapSubnav />);
    expect(screen.getByRole('link', { name: 'Queue' })).toHaveAttribute('href', '/gap');
    expect(screen.getByRole('link', { name: 'Hypotheses' })).toHaveAttribute('href', '/gap/hypotheses');
  ...
  it('marks the current tab with aria-current, and only that one', () => {
    ...
    render(<GapSubnav />);
    expect(screen.getByRole('link', { name: 'Hypotheses' })).toHaveAttribute('aria-current', 'page');
