// Batch 2 - Part B scheduler. Composes a fresh email per contact (To + Bcc +
// subject + inline proof frame) and Schedule-sends. Spread across two mornings:
//   Wed: node scripts/allentown-emails/compose-schedule-partB.mjs --date "Jun 10, 2026" --from 0 --to 18
//   Thu: node scripts/allentown-emails/compose-schedule-partB.mjs --date "Jun 11, 2026" --from 18 --to 35
// Each slice staggers every 2 min from 7:30 AM ET. --dry composes+screenshots without scheduling.

import { chromium } from 'playwright';
import { fileURLToPath } from 'url';
import fs from 'fs';
import path from 'path';
import { contacts, BCC } from './batch2-partB-contacts.mjs';
...
const DATE = argv('--date', 'Jun 10, 2026');
...
async function main() {
  ...
  async function doOne(c, timeStr, n) {
    ...
    if (DRY) {
      await page.screenshot({ path: path.resolve(REPO, `tmp/partB-dry-${n}.png`) });
      console.log(`DRY ${c.to}: composed + image (not scheduled)`);
  ...
  for (const c of slice) {
    ...
    try { if (await doOne(c, t, n)) ok++; }
      ...
      await page.screenshot({ path: path.resolve(REPO, `tmp/partB-error-${FROM + n}.png`) }).catch(() => {});
      for (let k = 0; k < 5; k++) { await page.keyboard.press('Escape').catch(() => {}); await sleep(300); }
      await waitClosed();
    }
  }
  console.log(`\nDone. ${DRY ? 'Dry-composed' : 'Scheduled'} ${ok}/${slice.length} for ${DATE}.`);
  await ctx.close();
}
