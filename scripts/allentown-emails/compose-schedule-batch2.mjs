// Batch 2 - Part A: compose a fresh email per vetted contact (To + Bcc + subject
// + inline proof frame) and Schedule-send for Tue Jun 9 2026, staggered every
// 2 min from 7:30 AM ET. Same proven flow as compose-schedule-tier1.
//
//   node scripts/allentown-emails/compose-schedule-batch2.mjs --dry --max 1
//   node scripts/allentown-emails/compose-schedule-batch2.mjs --max 1
//   node scripts/allentown-emails/compose-schedule-batch2.mjs

import { chromium } from 'playwright';
import { fileURLToPath } from 'url';
import fs from 'fs';
import path from 'path';
import { contacts, BCC } from './batch2-contacts.mjs';
...
async function main() {
  ...
  async function doOne(c, timeStr, n) {
    ...
    if (DRY) {
      await page.screenshot({ path: path.resolve(REPO, `tmp/batch2-dry-${n}.png`) });
  ...
  for (const c of list) {
    ...
    try { if (await doOne(c, t, n)) ok++; }
      ...
      await page.screenshot({ path: path.resolve(REPO, `tmp/batch2-error-${n}.png`) }).catch(() => {});
      for (let k = 0; k < 5; k++) { await page.keyboard.press('Escape').catch(() => {}); await sleep(300); }
      await waitClosed();
    }
  }
  console.log(`\nDone. ${DRY ? 'Dry-composed' : 'Scheduled'} ${ok}/${list.length}.`);
  await ctx.close();
}
