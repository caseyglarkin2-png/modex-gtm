...
const files = readdirSync(dir).filter(f => /^verdicts-10[1-4]\.jsonl$/.test(f));
...
for (const f of files) {
  for (const line of readFileSync(new URL(f, dir), 'utf8').split('\n')) {
    ...
    const props = { yardflow_tam: v.verdict, tam_source: 'web_research_2026-06-12_tail', tam_evaluated_at: now, tam_reason: (v.reason||'').slice(0,300) };
