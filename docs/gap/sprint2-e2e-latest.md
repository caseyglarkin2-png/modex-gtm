...
- PASS 1 seed: account, personas 31 (ready, director), 32 (ready, vp), 33 (suppressed), 2 triggers 10 days old, fixture accounts Dell + J.B. Hunt, sequence 15 with 2 draft queue rows (approved + draft) on run gap-e2e2-1790205746876-run
- PASS 2 hypothesize: signals.created=2 proposed=3 opsDraft=cmueqa2wx00097k30lnm69wb1 (yard_state_integrity) execDraft=cmueqa2x4000d7k30h5brthoo (hidden_capacity); fact cmueqa2xg000h7k3094yq271n linked (1 -> 2), relink already, unlink of cited cmueqa2wc00037k30dgcermbc refused signal_cited; 3 drafts submitted and approved
...
- PASS 4 replies: first poll created 1 filtered {auto_reply_subject: 1}, inbound hs:gap-e2e2-1790205746876-e1 on hs-thread:gap-e2e2-1790205746876-c1, notifications reply + filtered_inbound; second poll created 0 existing 2; watermark advanced; 2 fixture searches
- PASS 5 routing: run A mode shadow: 2 decisions {"enroll":1,"suppressed":1}, skips {"in_flight":1}; exec persona 32 -> enroll (target modex_queue, hypothesis cmueqa2x4000d7k30h5brthoo); do_not_contact persona -> suppressed / do_not_contact / blocked (R2-1); every row carries account + persona; queue ordered by priority; human action ok then already_acted, missing id not_found
- PASS 6 enroll rows: header + 1 row for GAP E2E Co gap-e2e2-1790205746876: sequence NOT BUILT, 0 contacts, 1 skip (E2E Exec gap-e2e2-1790205746876: modex_queue (no native sequence; secondary lane))
...
- PASS 8 stop: cancelDownstream(replied) marked 2 rows skipped with sequence_stopped:replied, 2 rows remain, rerun marks 0; routing run B: {"enroll":1,"suppressed":1,"reply_pending":1}, ready persona 31 -> reply_pending (rule id printed as observed)
