...
## 1. Identity: "Walmart" canonical account -- RESOLVED (2026-09-24)

- **Owner decision**: "Walmart Distribution Center" is NOT the canonical enterprise account. The canonical account is **Walmart Inc.**, domain walmart.com.
- **What was done**: HubSpot search confirmed exactly one deterministic company for domain=walmart.com (id 8536615003). Created `Account` "Walmart Inc." (id 1892, hubspot_company_id=8536615003), a resolved `CanonicalCompany`/`CanonicalAccountLink`, and an explicit `GapAccountAlias` (Walmart -> Walmart Inc.). "Walmart Distribution Center" (id 674) untouched, kept as a separate operational account.
- **Result**: re-ran the Inland26 reconciler; all 3 Walmart emails now resolve to Walmart Inc. as HYPOTHESIS_MISSING (accurate -- no GAP hypothesis exists for the historical send, none fabricated).
- **Link**: `docs/gap/inland26-dogfood-reconciliation.md`, `scripts/gap/dogfood-walmart-identity.ts`
