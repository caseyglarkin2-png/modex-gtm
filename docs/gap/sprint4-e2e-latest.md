...
- PASS 1 seed: account, personas 44/45/46 at example.com, facts cmuesttp (operator) + cmuesttp (public url), hypotheses cmuesttq/cmuesttr/cmuestts active citing both facts, family cmuestts v1 compiled pass (4 steps x 3 hypotheses, stub critic), three live modex enrollments each with a step-0 draft item
- PASS 2 ingest: enrollment ce0b0cf5 paused with the reply_pending marker (inbound gap-e2e4-1790210027344-msg-1), step-0 item skipped sequence_stopped:replied, 1 reply.ingested audit row; second ingest already_paused with no second row; the other two runs still active
- PASS 3 list: reply gap-e2e4-1790210027344-msg-1 listed: source inbound_message/gap-e2e4-1790210027344-msg-1, persona 44, hypothesis cmuesttq (network_standardization), enrollment paused, no suggestion, snippet is the plain text with no tag
- PASS 4 suggest: unconfirmed ai row cmuestu0 (created_by ai, problem_confirmed, quote verbatim) with the hypothesis still active, the enrollment still paused, no unsubscribe row, no resolution, no BID; second call idempotent (1 model call); a quote not in the text -> null, quote_not_found:0, reply.suggest_rejected audited, no row
- PASS 5 disposition: ai row cmuestu0 became the human's row (confirmed_by casey@freightroll.com, metadata.aiSuggestion matched=true), 2 BIDs confirmed, enrollment stopped (replied), hypothesis cmuesttq confirmed at 85 (60 email + 15 quote + 10 root cause) citing both BID ids, mirror skipped:gap_mirror_disabled with no mirror row; audit kinds disposition=[reply.suggested, disposition.recorded, disposition.effects], hypothesis has one hypothesis.resolved, enrollment has one enrollment.pause + one enrollment.stop + one enroll.live; resubmit on the resolved hypothesis -> hypothesis_not_active, same source on an active hypothesis -> duplicate_source (existingId); the reply left the undispositioned list and state=all carries dispositionId
- PASS 6 db truth: GAP_DISPOSITION_FROZEN refuses response_class, metadata and a confirmation revert on the confirmed row; GAP_BID_IMMUTABLE refuses a BID delete and a raw-language edit; correctBid inserted cmuestu3 superseding cmuestu1 (original untouched), selectConfirmedBids drops the old row and keeps the new plus the root cause; a second correction is already_superseded
- PASS 7 voicemail: voicemail on marcus+gap-e2e4-1790210027344@example.com recorded confirmed (row cmuestu3) with no stop, no resolution, no unsubscribe: enrollment 5da2073e still active, its draft item still draft, hypothesis still active; the table says keepsSequence; voicemail on channel email is refused call_only_class
- PASS 8 timing: timing on marcus+gap-e2e4-1790210027344@example.com confirmed with metadata.resumeAt 2026-11-02T14:00:00.000Z; hypothesis cmuesttr still active with no resolution; the run stopped (replied); assembleRoutingInputs for persona 45 reads comms.lastDisposition = timing with that resumeAt
- PASS 9 dnc: do_not_contact on dana+gap-e2e4-1790210027344@example.com: UnsubscribedEmail row (reason "do_not_contact disposition"), Persona.do_not_contact true (the other two false), run d1f86483 stopped (dnc) with its item skipped sequence_stopped:dnc, mirror skipped:gap_mirror_disabled, hypothesis untouched; a shadow enrollFromDecision is refused suppressed on leg unsubscribed (the first leg that hits; the persona flag alone is the modex_do_not_contact leg) and audited enroll.refused
- PASS 10 brief: brief for persona 44: FACT block observation cites both facts and the signals list carries both (one with the public url), the confirmed disposition with the buyer's words is the only disposition, zero open BIDs (all confirmed or superseded), the falsification question is the suggested question; the dnc persona's brief says doNotContact
- PASS 11 agent row: agent (cron) problem_rejected on marcus+gap-e2e4-1790210027344@example.com stored unconfirmed with effects none and only disposition.recorded audited; hypothesis cmuesttr still active with no resolution; a human adopting it through aiSuggestionId is refused ai_suggestion_not_adoptable (only created_by ai rows adopt: NO human confirm path for agent rows exists, named debt), an agent passing aiSuggestionId is refused agent_cannot_confirm
...
- enrollments: 3
- aiSuggestionRowId: cmuestu06002r7kccuo2d1tb6
- dispositionId: cmuestu06002r7kccuo2d1tb6
- confirmedBidIds: cmuestu14002v7kccpm7vd5ve,cmuestu16002x7kcc1dg4dxda
- resolutionConfidence: 85
- correctedBidId: cmuestu3h00377kcczrt8lnyr
- timingDispositionId: cmuestu3y003d7kccbq2gvyry
- dncDispositionId: cmuestu4v003i7kcclbqh1op1
- agentDispositionId: cmuestu63003p7kcc4799xho7
